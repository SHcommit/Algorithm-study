...
/// 첫 트라이
/// 조합으로 키벨류값 전부 만들고, 이유는 도무지 이렇게밖에 빨리하지 않을수 없을거 같았음..
/// 물론 매번 사용자 정보에 따라 2*2*2*2 이렇게 16가지의 키들을 추가로 만든 후에 값을 증가시키는건데
///
/// 나중에 값 비교할때 보단 더 빠르게 딕셔너리니까 O(1)느낌으로 비겨할수있을거같아서 선정.
///
