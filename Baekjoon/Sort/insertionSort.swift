 ...
 *
 * 추가적으로 정렬되어야 할 index 1 원소 '3'을 우선 newItem에 저장시킨다.
 *
 ...
 *  이 과정이 계속 반복된다.
 *
 *  j -=1이되면서, 이전원소의 위치로 newItem이 이동했을 때 해당 index를 기준으로 또 그 이전 index에 위치한 원소와 크기 비교를 한다. ( array[j -1] > newItem)
 * 또는 언제까지?
 * j >0 일 때까지 반복을 한다.
 * 이때까지 while문이 돌아간 다는 것은 주어진 원소newItem이 배열에서 크기가 가장 작다는 의미이다.
 ...
 * 이 for in 구문이 끝난이후에 주어진 배열은 오름차순으로 정렬 될 것이다.
 *
 * @property j
